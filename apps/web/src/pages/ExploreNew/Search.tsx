...
const ExploreNew = () => {
  const { entities, entitiesLoading } = useExplorerContext()

  return (
    <Flex direction='column' w='100%' maw='100vw' h='100%' align='center'>
      <Box w='90%' mt={40} mx='auto'>
        <FeaturedEntities entities={entitiesLoading ? [] : entities.slice(0, 8)} loading={entitiesLoading} />
      </Box>
      ...
      <Flex w='90%' mt={10} wrap='wrap' gap='md' mx='auto'>
        <EntitiesTable entities={entitiesLoading ? [] : entities.slice(8)} />
      </Flex>
    </Flex>
  )
}
