...
export const useWallet = (): UseWalletProps => {
  ...
  const executeTxBody = async ({ txBody }: { txBody: Uint8Array }) => {
    const transactData = await context.signXWallet.initTransaction(1, txBody, context.wallet as any)
  ...
  const execute = async ({ data, transactionConfig }: ExecuteProps) => {
    const transactData = await context.signXWallet.initTransaction(1, data.messages, context.wallet as any)
