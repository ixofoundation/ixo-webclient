...
import { IconAgentCapability } from 'components/IconPaths'
...
const GroupsCard: React.FC = () => {
  ...
  if (participatingDAOGroups.length === 0) {
    if (daoGroups.length === 0) {
      return (
        <Card label={`Loading Membership data`} icon={IconAgentCapability}>
  ...
  return (
    <Card label={`Member of ${participatingDAOGroups.length} Groups`} icon={IconAgentCapability}>
