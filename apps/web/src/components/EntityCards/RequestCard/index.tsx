...
const RequestCard = ({ entityName, requestName, requestImage, requestDescription, did }: RequestCardProps) => {
  return (
    <NavLink to={`/entity/${did}`} style={{ padding: 'none' }}>
      <Card shadow='sm' padding='lg' radius='md' withBorder bg={'#103449'} style={{ borderColor: '#103449' }}>
        <Card.Section>
          <Image src={requestImage} height={160} alt={requestName} />
        </Card.Section>
        ...
        <Text size='sm' my={'md'} c='white' h='50px'>
          {truncate(requestDescription, { length: 90 })}
        </Text>

        <Divider color='#213E59' />

        <Flex justify='space-between' mt='sm'>
          <Badge fw={400} size='lg' leftSection={<img src="/assets/tokens/ixo.svg" width={20} height={20}  />} bg='#213E59'>
