...
const updateNativeTokenBalances = async (address: string, chainInfo: KeplrChainInfo) => {
    ...
    const coins = await Promise.all(balances.map((coin) => getTreasuryCoinByDenom(address, coin, chainInfo.rpc)).filter((v) => v !== undefined)) as TTreasuryCoinModel[]
...
const getEntityAccountBalances = async ({ accounts }: { accounts: EntityAccount[] }) => {
    return accounts.reduce(async (acc, { address, name }) => {
        ...
        return ({
            ...acc,
            [address]: { address, name, type: 'entity', network: 'ixo Network', coins: await updateNativeTokenBalances(address, chainInfo) },
        })
    }, {});
}

const getLinkedAccountBalances = async ({ linkedEntity }: { linkedEntity: LinkedEntity[] }) => {
    return linkedEntity.filter((item) => item.type === 'BlockchainAccount').reduce(async (acc, { id, relationship }) => {
        const chainInfo = await determineChainFromAddress(id)
        if (!chainInfo) return
        return ({
            ...acc,
            [id]: { address: id, name: truncateString(id, 15), type: 'linked', network: relationship, coins: await updateNativeTokenBalances(id, chainInfo) },
        })
    }, {})
}
...
const getMemberGroupAccountBalances = async ({ daoGroups }: { daoGroups: TEntityModel["daoGroups"] }) => {
    return Object.values(daoGroups ?? {}).reduce(async (acc, group) => {
        ...
        return ({
            ...acc,
            [group.coreAddress]: { address: group.coreAddress, name: group.config.name, type: 'group', network: 'ixo Network', coins: await updateNativeTokenBalances(group.coreAddress, chainInfo) },
        })
    }, {});
}
...
export function useCurrentEntityTreasury({ entityId = "" }) {
    ...
    useEffect(() => {
        getEntityAccountBalances({ accounts: entity.accounts }).then(response => setAccounts(prev => ({ ...prev, ...response })))
    }, [entity.accounts])
    ...
    useEffect(() => {
        getLinkedAccountBalances({ linkedEntity: entity.linkedEntity }).then(response => setAccounts(prev => ({ ...prev, ...response })))
    }, [entity.linkedEntity])

    useEffect(() => {
        getMemberGroupAccountBalances({ daoGroups: findKeyValuePairs(entity?.daoGroups ?? {}, "type", "membership") }).then(response => {
            return response
        }).then(response => setAccounts(prev => ({ ...prev, ...response })))
    }, [entity.daoGroups])
