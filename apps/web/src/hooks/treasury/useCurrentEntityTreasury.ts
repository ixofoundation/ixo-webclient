...
const getStakingGroupBalance = async ({ stakingGroup }: { stakingGroup: TDAOGroupModel }): Promise<{ [key: string]: TTreasuryCoinModel } | undefined> => {
...
const getEntityAccountBalances = async ({ accounts }: { accounts: EntityAccount[] }): Promise<{ [address: string]: TTreasuryAccountModel }> => {
...
const getLinkedAccountBalances = async ({ linkedEntity }: { linkedEntity: LinkedEntity[] }): Promise<{ [address: string]: TTreasuryAccountModel }> => {
  const results = await Promise.all(
    linkedEntity
...
const getStakingAccountBalances = async ({ daoGroups }: { daoGroups: TEntityModel['daoGroups'] }): Promise<{ [address: string]: TTreasuryAccountModel }> => {
  const results = await Promise.all(
...
const getMemberGroupAccountBalances = async ({ daoGroups }: { daoGroups: TEntityModel['daoGroups'] }): Promise<{ [address: string]: TTreasuryAccountModel }> => {
  const results = await Promise.all(
    Object.values(daoGroups ?? {}).map(async (group) => {
