    ...
    const { refetch } = useEntityQuery({
        variables: { id: did }, onCompleted: async (data) => {
            if (data.entity?.service && data.entity.settings) {
                ...
                const administratorResource = data.entity.linkedResource.find((resource: any) => resource.id === "{id}#administrator")
                ...
                dispatch(updateEntityAction({ ...data.entity, ...entity, ...page, profile, creator, claim, daoGroups, tags, administrator}))
            }
        }
    })
