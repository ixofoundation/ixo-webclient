...
export const getHeaderSchema = (filterSector: string, headerSchema: Schema): SchemaBase => {
  const headerOverride = headerSchema.overrides.find((override) => filterSector === override.ddoTag)

  return headerOverride || headerSchema
}

export const getHeaderTabButtons = (entityType: EntityType, entityTitle: string): any => {
  const tabButtons = [
    {
