...
export const StyledTableCell = styled.div<StyledTableCellProps>`
  width: 20%;
  color: ${(props: any): string => (props.header === 'buySell' ? (props.cellType ? '#6FCF97' : '#E2223B') : 'inherit')};
  font-weight: bold;
  &:first-child {
    padding-left: 2em;
  }
...
export const StyledOptionCell = styled.div`
  color: ${(props: any): string => (props.header === 'option' ? extractColor(props.option) : 'inherit')};
  font-weight: bold;
`
