...
export default class BarChartProjects extends React.Component<ParentProps, State> {
	...
	populateXaxisLabels(hoursPerBucket: number) {
		...
		for (let i = 0; i < this.state.totalBars; i += 1) {
			...
			if (i % 10  === 0) {
				let theTime = compEnd.clone().subtract(i, 'days');
				labelArray.push(theTime.format('D MMM'));
			}
		}

		let firstTime = compEnd.clone().subtract(this.state.totalBars, 'days');
	...
	getBarDate(index: number) {
		...
		let theTime = compEnd.clone().subtract(theDiff, 'hours');
