...
const BLOCKSYNC_API = process.env.NEXT_PUBLIC_BLOCK_SYNC_URL
...
export const getBondDetail =
  ...
  (dispatch: Dispatch): GetBondDetailAction => {
    ...
    const bondRequest = Axios.get(`${process.env.NEXT_PUBLIC_GAIA_URL}/bonds/${bondDid}`, {
      transformResponse: [
        (response: string): any => {
...
export const getPriceHistory =
  ...
  (dispatch: Dispatch): GetPriceHistoryAction => {
    return dispatch({
      ...
      payload: Axios.get(`${process.env.NEXT_PUBLIC_BLOCK_SYNC_URL}/api/bonds/getPriceHistoryByBondDid/${bondDid}`)
        .then((res) => res.data)
