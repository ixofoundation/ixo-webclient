...
export const updateExistingEntityError =
  (): UpdateExistingEntityErrorAction => ({
...
export const fetchExistingEntity =
  ...
  (dispatch: Dispatch, getState: () => RootState): any => {
    ...
    const fetchEntity: Promise<ApiListedEntity> =
      blockSyncApi.project.getProjectByProjectDid(did)

    const fetchContent = (
      key: string,
      cellNodeEndpoint: string,
    ): Promise<ApiResource> =>
      blockSyncApi.project.fetchPublic(
        key,
        cellNodeEndpoint,
    ...
    fetchEntity
      .then((apiEntity: ApiListedEntity): any => {
        let cellNodeEndpoint =
          apiEntity.data.nodes.items.find(
            (item) => item['@type'] === 'CellNode',
        ...
        return fetchContent(apiEntity.data.page.cid, cellNodeEndpoint).then(
          (resourceData: ApiResource) => {
            ...
            if (apiEntity.data['@type'] === EntityType.Template) {
                ...
                questions: content.forms.reduce((obj, item) => {
                  ...
                  return {
                    ...
                    [uuid]: {
                      ...
                      initialValue:
                        item['schema']['properties'][uuid]['default'],
                      itemValues:
                        item['schema']['properties'][uuid]['items']['enum'],
                      itemLabels:
                        item['schema']['properties'][uuid]['items'][
                          'enumNames'
                        ],
              ...
              const { header, body, images, profiles, social, embedded } =
                content
              ...
              dispatch(
                importEntityClaims({
                  entityClaims: entityClaims.items.reduce(
                    (obj, entityClaim) => {
                      const uuid = uuidv4()
                      identifiers.push(uuid)
                      ...
                      return {
                        ...
                        [uuid]: {
                          ...
                          agentRoles: entityClaim.agents.reduce(
                            (obj, agent) => {
                              const uuid = uuidv4()
                              identifiers.push(uuid)
