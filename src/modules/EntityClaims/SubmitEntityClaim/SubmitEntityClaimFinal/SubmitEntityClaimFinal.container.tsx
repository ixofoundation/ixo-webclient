...
import { createEntityClaim, goToQuestionNumber } from '../SubmitEntityClaim.actions'
...
class SummaryContainer extends React.Component<Props, State> {
  ...
  render(): JSX.Element {
    const { entityDid, questions, answersComplete, creating, created, error, handleCreateClaim } = this.props

    if (this.state.showForm || !answersComplete) {
      return <Redirect to={`/projects/${entityDid}/overview/action/new_claim/form`} />
    }
...
export const SummaryContainerConnected = connect(mapStateToProps, mapDispatchToProps)(SummaryContainer)
