    ...
    payload: Promise.all([oldTransactionReq, transactionReq, priceReq]).then(
      Axios.spread((...responses) => {
        ...
        return transactions.map((data) => {
          ...
          if (buySell) {
            isMyTX = transaction.tx?.body?.messages[0]['buyer_did'].includes(
              userDid,
            )
          }
          const price =
            priceHistory.find(
              (his) => {
                return (
                  Math.abs(moment(his.time).diff(transaction.timestamp)) < 1000
                )
              },
              // moment(his.time).diff(transaction.timestamp) < 1,
            )?.price ??
            priceHistory.filter((his) => transaction.timestamp > his.time).pop()
              ?.price ??
            0
...
export const getAlphaHistory = (bondDid) => (
  dispatch: Dispatch,
): GetAlphaHistoryAction => {
  return dispatch({
...
export const getWithdrawHistory = (bondDid) => (
  dispatch: Dispatch,
): GetWithdrawHistoryAction => {
  const withdrawReserveReq = Axios.get(
    `${BLOCKSYNC_API}/api/bond/get/withdraw/reserve/bybonddid/${bondDid}`,
  )

  const withdrawShareReq = Axios.get(
    `${BLOCKSYNC_API}/api/bond/get/withdraw/share/bybondid/${bondDid}`,
  )
        ...
        return withdrawTransactions
          ...
          .map((history) => {
            ...
            if (isWithdrawShare) {
              ...
              const value = attributes.find(({ key }) => key === 'amount').value
